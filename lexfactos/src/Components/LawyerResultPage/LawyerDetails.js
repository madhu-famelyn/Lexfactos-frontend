...
import { FaPhone, FaCalendarAlt, FaShareAlt, FaBookmark } from "react-icons/fa";
    ...
    const fetchLawyer = async () => {
      try {
        ...
        const res = await axios.get(
          `http://127.0.0.1:8000/get-all-details/lawyers/all-details/${id}`
        );
        setLawyer(res.data);
      } catch (err) {
...
<div className="lawyer-profile-header">
  ...
  <div className="lawyer-profile-left">
    <img src={lawyer.photo} alt={lawyer.full_name} className="lawyer-profile-photo" />
    ...
    <div className="lawyer-profile-info">
      ...
      <p className="lawyer-chamber-name">Kumar & Associates Law Chambers</p>
      ...
      <div className="lawyer-rating-section">
        ...
        <span className="lawyer-reviews">({lawyer.reviews_count || "245 reviews"})</span>
      </div>
        ...
        <div className="lawyer-left-column">
          ...
          <section className="lawyer-about-section">
            ...
            <p className="lawyer-about-text">
              Advocate Rajesh Kumar is a distinguished Senior Advocate with over
              20 years of extensive experience in various fields of law. He has
              been practicing in the Delhi High Court and Supreme Court of India,
              specializing in Criminal Law, Civil Litigation, Corporate Law, and
              Family Disputes.
            </p>
          </section>
    ...
    {lawyer.profile?.education?.map((edu, idx) => (
      <li key={idx}>
        ...
        <span className="details">
          {edu.graduation_year} {edu.additional_info && `• ${edu.additional_info}`}
        </span>
      </li>
    ))}
  </ul>
...
/>
      ...
      <section>
        <h2>Bar Details</h2>
        <ul>
          {lawyer.profile?.bar_details?.map((bar, idx) => (
            <li key={idx}>
              {bar.bar_license_number} – {bar.bar_association_name}, {bar.city}, {bar.state}
            </li>
          ))}
        </ul>
      </section> */}
